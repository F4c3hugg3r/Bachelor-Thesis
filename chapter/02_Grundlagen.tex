...
In diesem Kapitel werden die nötigen Grundlagen zum Verständnis des Portscannings in Form von \texttt{SYN}-Scans, sowie 
das nötige Wissen über Netzwerkkommunikation, die genutzten Technologien und Linux-Schnittstellen vermittelt.
Des Weiteren wird auf asynchrone Programmierung eingegangen, um das Verständnis für das nachfolgende Konzept 
der Implementierung zu schaffen.
...
Netzwerken dienen das IP-Protokoll und IP-Adressen der Identifikation der Maschine im Netzwerk, während die genaue 
Adressierung der spezifischen Anwendungen durch sogenannte Ports bzw. die Portnummer bestimmt wird \cite{Chapter_4._Port_Scanning_Overview_|_Nmap_Network_Scanning}. 
...
\begin{itemize} 
	\item \textbf{Offen:} Eine Anwendung lauscht auf dem Port und akzeptiert eingehende gültige 
	TCP oder UDP Anfragen \cite{Lyon_2010}.
	\item \textbf{Geschlossen / Gefiltert:} Der mit dem Port verbundene Dienst ist ansprechbar, akzeptiert jedoch keine eingehenden Verbindungen.
	Oft wird eine ICMP (Fehler) Antwort zurückgegeben oder es erfolgt keine Antwort, da beispielsweise kein Service für diesen Port
...
der Aufbau des TCP-Headers (\cref{fig:tcp_header}) und der Mechanismus zum Verbindungsaufbau (\cref{fig:tcp_handshake}) entscheidend. 
...
Da das TCP-Protokoll Daten als Datenstrom statt in einzelnen Nachrichten versendet, 
wird vorab eine Verbindung über den sogenannten \eng{Three-Way-Handshake} aufgebaut \cite{Wendzel_2021} S71/72. 
Bei diesem werden TCP-Pakete mit jeweils unterschiedlichen Werten in den \eng{Control Bits} (\eng{Flags}) 
des TCP-Headers nach dem in \cref{fig:tcp_handshake} beschriebenen Muster ausgetauscht.
...
vollständiges Profil möglicher Schwachstellen des Zielsystems zu erlangen, weshalb es sich besonders für das \eng{Penetration Testing} eignet.
...
die Verbreitung von Schwachstellen zu sammeln \cite{Durumeric_Wustrow_Halderman_182948}.
\end{itemize} 

\subsection{SYN-Scanning}
Für einen \eng{Half-Open} \texttt{SYN}-Scan wie er in dieser Arbeit behandelt wird, sind lediglich die ersten beiden Schritte des in \cref{fig:tcp_handshake} 
dargestellten Verbindungsablaufes relevant. Es wird davon Gebrauch gemacht, dass bereits und ausschließlich eine \texttt{SYN/ACK}-Antwort den Port als offen klassifiziert, 
was den weiteren Verbindungsaufbau irrelevant macht \cite{Lyon_2010}. Um den Scan zu verschleiern und den Verbindungsversuch dennoch korrekt
zu beenden, kann anschließend noch ein Paket, bei welchem die \texttt{RST}-Flag der \eng{Control Bits} gesetzt ist, gesendet werden \cite{Lyon_2010}.

Um antwortende \eng{Hosts} effizient zu identifizieren, wird das Prinzip des \texttt{SYN}-Cookies adaptiert \cite{Durumeric_Wustrow_Halderman_182948}, welches ursprünglich als 
...
Informationen unter Verwendung eines \eng{Hash}-Algorithmus (z. B. \eng{Keyed SipHash} \cite{SipHash_a_short_input_PRF_The_Linux_Kernel_documentation}) kodiert 
...
Entscheidend für den Scanner sind demnach die in \cref{tab:TCP-header-fields} aufgeführten Header Felder. 
...
\subsection{Linux}
Linux ist ein \eng{Open-Source}-Betriebssystem-Kernel \cite[S.~1]{Kerrisk_2018}, welcher aufgrund neuartiger Subsysteme, wie beispielsweise dem in \cref{Grundlagen.eBPF} vorgestellten \texttt{eBPF} oder \texttt{XDP} (\cref{Grundlagen.XDP}), eine programmierbare Paketverarbeitung nahe an der Hardware ermöglicht. 
...
\subsection{Raw-Sockets und Adressfamilien}
...
Die Adressfamilien definieren dabei die Interpretation der Adressen und die Ebene des Zugriffs \cite{address_families7_linux_manpage}. 
Der Linux-Kernel stellt diverse Adressfamilien bereit. Zum Verständnis, im Rahmen dieses Projektes, sind folgende Varianten von zentraler 
Bedeutung:

\begin{itemize} 
	\itemsep 0pt
	\item \textbf{\texttt{AF\_INET}:} Diese Familie operiert auf Layer 3 der IP-Ebene \cite{raw7_linux_manpage}. Bei Nutzung von \eng{Raw-Sockets} fügt der Kernel 
    standardmäßig den IP-Header hinzu und übernimmt das vollständige Routing zur korrekten Netzwerkschnittstelle \cite[S.~1202]{Kerrisk_2018}.
 	\item \textbf{\texttt{AF\_PACKET}:} Diese Familie ermöglicht direkten Zugriff auf Layer 2 (Ethernet-Ebene). 
    ...
    \item \textbf{\texttt{AF\_XDP}:} Hierbei handelt es sich um eine speziell für Hochleistungsanwendungen optimierte Adressfamilie.
...
wurde die Technologie erweitert, sodass grundlegend neue Möglichkeiten außerhalb des reinen Filterns von Paketen erschlossen wurden. 

\texttt{eBPF} ist eine im Linux-Kernel integrierte virtuelle Maschine (VM), die es erlaubt, benutzerdefinierten 
\eng{Bytecode} sicher und effizient im \eng{Kernel}-Kontext (siehe \cref{fig:rx_xdp}) auszuführen, ohne Kernel-Module schreiben oder den Kernel 
...
So wird undefiniertes Verhalten durch Fehler wie beispielsweise Endlosschleifen oder falsche Speicherzugriffe strikt vermieden.
...
Dies sind generische Schlüssel-Wert-Speicher, die sowohl von verschiedenen eBPF-Programmen 
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{pictures/RX_Kernel_nr.drawio.pdf}
...
gesamten Netzwerk-Stack. Erst danach erreichen die Daten den \eng{User-Space}. Dies erfordert CPU- und speicheraufwendige 
\eng{Context Switches} zwischen \eng{Kernel-} und \eng{User-Space}, sowie die Allokation 
...
\texttt{XDP} greift vor dieser Allokation ein (siehe \cref{fig:rx_xdp}). Tests zeigen, dass \texttt{XDP} auf einem einzelnen CPU-Kern bis zu 
...
\begin{itemize} 
	\item \textbf{\eng{Native Mode} (\eng{DRV}):} Dies ist der Standardmodus für Hochleistungsanwendungen. 
	...
	\item \textbf{\eng{Offloaded Mode} (\eng{HW}):} Hierbei wird das \texttt{eBPF}-Programm vom Kernel auf die 
	...
	die Nutzung einer sogenannten \eng{SmartNIC} voraus. 
	\item \textbf{\eng{Generic Mode} (\eng{SKB}):} Dieser Modus dient der Kompatibilität. Wenn ein Treiber \texttt{XDP} 
...
Das Hauptaugenmerk der Sprache liegt auf der Sicherheit, wobei auch \eng{Performance} und Nebenläufigkeit 
zunehmend an Bedeutung gewinnen \cite{Bugden_Alahmar_2022}. Rust vereint dabei als erste Sprache
Speichersicherheits-Konzepte höherer Abstraktionsebenen mit der direkten Ressourcenkontrolle 
systemnaher Sprachen \cite{Jung_Jourdan_Krebbers_Dreyer_2021}.
...
Budgen et al. \cite{Bugden_Alahmar_2022} führen weitere Konzepte auf, wie das 
...
sie eine Struktur zurückgeben, die entweder den gewünschten Wert x als \texttt{Some(x)} oder \texttt{None} enthält. 
Außerdem muss Zeiger-Arithmetik in sogenannte \texttt{unsafe}-Blöcke ausgelagert werden.
Diese dienen als Möglichkeit zur Umgehung der anderen Konzepte. Sie lösen
...
Die Beseitigung dieser Fehlerquellen macht Rust zu einer guten Wahl für sowohl nebenläufige als auch parallele Programmierung. 
...
indem sie während möglicher Wartezeiten z. B. bei \eng{I/O}-Operationen\footnote{Ein-/Ausgabe-Operationen} für andere
...
\subsubsection{Rusts Konzepte zur Performance-Steigerung}
...
Diese Konzepte und vor allem die Prüfung der in \cref{rust.concepts} vorgestellten Konzepte zur 

...
Port-Scanning, als Art des Netzwerk-Scannings, ist eines der fundamentalen Verfahren in der Netzwerksicherheit zum Auffinden von 
...
Für einen \eng{Half-Open} \texttt{SYN}-Scan wie er in dieser Arbeit behandelt wird, sind lediglich die ersten beiden Schritte des Verbindungsablaufes
...
Informationen unter Verwendung eines \eng{Hash}-Algorithmus (z. B. \eng{Keyed SipHash}) kodiert 
...
Linux ist ein \eng{Open-Source}-Betriebssystem-Kernel \cite[S.~1]{Kerrisk_2018}, welcher aufgrund neuartiger Subsysteme (wie beispielsweise \texttt{eBPF} (siehe \ref{Grundlagen.eBPF}) oder
 ...
 Dies ist für die Entwicklung eines Hochleistungs-Scanners von großem Vorteil.
...
Dies ist für \eng{Half-Open} Port-Scanner essenziell, um individuelle Pakete zu generieren, ohne dass der Kernel 
...
\begin{itemize} 
	...
	Anwendungen können rohe \eng{Ethernet-Frames} lesen und schreiben. Dies bietet die vollständige Kontrolle
	über die Paketerstellung. 
	% TODO wie weit durch Kernel geschickt? Was ist mit V1/V2/V3?
	\item \textbf{\texttt{AF\_XDP} (Sehr Performant):} Hierbei handelt es sich um eine für Hochleistungspaketverarbeitung optimierte \eng{Address Family}. 
...
\eng{Bytecode} sicher und effizient im \eng{Kernel}-Kontext auszuführen, ohne Kernel-Module schreiben oder den Kernel 
...
komplexer Metadatenstrukturen (\texttt{sk\_buff}) \footnote{\eng{Socket Buffer}} \cite{Høiland-Jørgensen_Brouer_Borkmann_Fastabend_Herbert_Ahern_Miller_2018}\cite{Zhang_Shu_Chen_Xie_2024}.
...
\begin{itemize} 
	...
	Trotzdem profitiert es von effizienteren, sperrfreien \eng{Ring-Buffer}-Strukturen (<- TODO bessere Argumente)
\end{itemize}
...
Außerdem müssen \eng{Pointer}-Arithmetik in sogenannte \texttt{unsafe}-Blöcke ausgelagert werden.

...
In diesem Kapitel werden die nötigen Grundlagen zum Verständnis des Portscanning in Form von \texttt{SYN}-Scans, sowie 
...
Netzwerken dient das IP-Protokoll und die IP-Adressen zur Identifikation der Maschine im Netzwerk, während die genaue 
...
Der gesamte Kommunikations-Endpunkt wird \textit{Socket} genannt. TODO cite [Computernetzwerke S108]. 
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{\textit{Three-Way-Handshake} zum Aufbau einer TCP-Verbindung \cite{Wendzel_2021}.}
    \label{fig:tcp_handshake}
\end{figure}

Da das TCP Protokoll Daten als Datenstrom (\textit{Stream}) statt einzeln (\textit{Message}) versendet, 
...
Portscanning, als Art des Netzwerk-Scannings, ist eines der fundamentalen Verfahren in der Netzwerksicherheit zum Auffinden von 
potenziellen Schwachstellen \cite{Lyon_2010}. Ein Portscanner verschickt Pakete an ein Zielsystem und
...
\begin{itemize} 
	\itemsep 0pt
	\item \textbf{Vertikales Scannen:} Hierbei wird ein einzelner Ziel-Host (TODO Host erklären) auf eine Vielzahl von Ports (oft alle 65535) gescannt, um ein möglichst
vollständiges Profil möglicher Schwachstellen des Zielsystems zu erlangen und somit eher für das Penetrationtesting geeignet ist.
...
Für einen \textit{half-open} \texttt{SYN}-Scan wie er in dieser Arbeit behandelt wird, sind lediglich die ersten beiden Schritte des Verbindungsablaufes
relevant. Es wird davon gebraucht gemacht, dass bereits und ausschließlich eine \texttt{SYN/ACK} Antwort den Port als offen klassifiziert, 
...
abzuschießen, kann anschließend noch ein Paket, bei welchem die \texttt{RST} Flag der \textit{Control Bits} gesetzt ist gesendet werden \cite{Lyon_2010}.

Um antwortende Hosts effizient zu identifizieren, wird das Prinzip des \texttt{SYN}-Cookies adaptiert \cite{Durumeric_Wustrow_Halderman}, welches ursprünglich als 
Abwehrmechanismus gegen \textit{Denial-of-Service}-Angriffe spezifiziert wurde \cite[S. 8]{Eddy_2007}. Dafür werden verbindungsspezifische 
Informationen unter Verwendung eines Hash-Algorithmus (z. B. keyed SipHash) kodiert 
und als \textit{Sequence Number} in den TCP-Header des ausgehenden \texttt{SYN}-Pakets eingetragen.
Antwortet ein Ziel-Host mit einem \texttt{SYN-ACK}-Paket, so enthält dessen \textit{Acknowledgment Number} gemäß TCP-Spezifikation den inkrementierten 
...
Da die Validierung einer Antwort somit rein mathematisch erfolgt und keine Speicherung in - oder Zugriff auf eine lokale Zustandstabelle benötigt, 
wird eine sowohl zeitliche als auch logische Entkopplung von Sende- und Empfangsprozessen erwirkt, was wiederum eine asynchrone 
...
\textit{Source Port} & Beschreibt den genutzten Port des Ausgangsdienstes. \\ \hline
\textit{Destination Port} &  Beschreibt den zu scannenden Port des Zielsystems. \\ \hline
\textit{Sequence Number} & Wird zur Speicherung des \texttt{SYN}-Cookies genutzt. \\ \hline
\textit{Acknowledgement Number} & Wird zum Abrufen des \texttt{SYN}-Cookies genutzt. \\ \hline
\textit{Control Bits (Flags)} & Wird für die verschiedenen Phasen des Verbindungsaufbaues angepasst oder ausgelesen.  \\ \hline
\end{tabularx}
...
\subsection{Linux}
Linux ist ein Open-Source Betriebssystem-Kernel, welcher aufgrund neuartiger Subsysteme (wie beispielsweise eBPF (siehe \ref{Grundlagen.eBPF}) oder
 XDP (siehe \ref{Grundlagen.XDP})), eine 
...
\begin{itemize} 
	\itemsep 0pt
	\item \textbf{Kernel Space:} Hier läuft der Kern des Betriebssystems mit vollem Zugriff 
	auf die Hardware und den Speicher. Treiber und der Netzwerk-Stack operieren auf dieser Ebene. 
	\item \textbf{User Space:} Hier laufen reguläre Anwendungen in isolierten Speicherbereichen. 
	Diese haben keinen direkten Zugriff auf den \textit{Kernel Space}. 
\end{itemize} 
Die Kommunikation zwischen diesen Ebenen erfolgt über \textit{System Calls}. Jeder Wechsel (\textit{Context Switch}) 
zwischen \textit{User} und \textit{Kernel Space}, sowie das Kopieren von Daten zwischen diesen 
Speicherbereichen, erzeugt \textit{Overhead}. Beim Versenden und Empfangen sehr vieler Pakete 
summiert sich dieser \textit{Overhead}, da jedes Paket im Normalfall sowohl \textit{Kernel Space}, als auch \textit{User Space}, durchschreitet [TODO].
...
Dies ist für \textit{half-open} Port-Scanner essenziell, um individuelle Pakete zu generieren, ohne dass der Kernel 
...
\begin{itemize} 
	\itemsep 0pt
	\item \textbf{\texttt{AF\_INET} (Standard):} \texttt{AF\_INET} Operiert auf Layer 3 (IP Ebene), deshalb fügt der Kernel den IP-Header hinzu. 	
	Auch das Routing wird (TODO vollständig?) durch den Kernel vorgenommen. 
 	\item \textbf{\texttt{AF\_PACKET} (Performant):} Diese Familie ermöglicht direkten Zugriff auf Layer 2 (Ethernet-Ebene). 
	Anwendungen können rohe Ethernet-Frames lesen und schreiben. Dies bietet die vollständige Kontrolle
	über die Paketerstellung. 
	% TODO wie weit durch Kernel geschickt? Was ist mit V1/V2/V3?
	\item \textbf{\texttt{AF\_XDP} (Sehr Performant):} Hierbei handelt es sich um eine für Hochleistungspaketverarbeitung optimierte Address-Familie. 
	Sie ermöglicht das Senden und Empfangen von Paketen unter fast vollständiger Umgehung des Kernel-Stacks. 
\end{itemize}

\subsection{Erweiterte Berkley Packet Filter (\texttt{eBPF})} \label{Grundlagen.eBPF}
Ursprünglich als \textit{Berkeley Packet Filter} (\texttt{BPF}) für Werkzeuge wie \textit{tcpdump} entwickelt, um Pakete effizient zu filtern \cite{260309}, 
...
Bytecode sicher und effizient im \textit{Kernel}-Kontext auszuführen, ohne Kernel-Module schreiben oder den Kernel 
neu kompilieren zu müssen \cite{Nishanth_Reddy_Pinnapareddy_2025} S207. \texttt{eBPF}-Programme werden zur Laufzeit 
durch einen \textit{JIT-Compiler} (\textit{Just-In-Time}) in native Maschinensprache übersetzt. Ein \textit{Verifier} stellt 
...
% TODO \texttt{bpf} Maps und \texttt{RingBuf} erklären!!!
% \cite{Zhang_Shu_Chen_Xie_2024}

Für einen \texttt{SYN}-Scanner ist \texttt{eBPF} nützlich, da es ermöglicht, eingehende Antwortpakete (\texttt{SYN-ACK}) extrem früh 
zu filtern und an den \textit{User Space} weiterzuleiten, bevor teure Speicherstrukturen des Kernels angelegt werden. 
So werden nur relevante Daten an den \textit{User Space} weitergereicht. 
...
über dieselbe Schnittstelle zurücksenden (\texttt{XDP\_TX}) oder an eine andere CPU bzw. einen \textit{Userspace-Socket} 
...
gesamten Netzwerk-Stack. Erst danach erreichen die Daten den \textit{User Space}. Dies erfordert CPU und Speicher-
aufwendige Kontextwechsel (\textit{Context Switches}) zwischen \textit{Kernel-} und \textit{User-Mode}, sowie die Allokation 
komplexer Metadatenstrukturen (\texttt{sk\_buff}) \footnote{\textit{Socket Buffer}} \cite{Høiland-Jørgensen_Brouer_Borkmann_Fastabend_Herbert_Ahern_Miller_2018}\cite{Zhang_Shu_Chen_Xie_2024}.
...
% TODO Pfeil vom ebpf Programm muss orange sein, Queuing und forwarding so stehen lassen?
\begin{figure}[htbp]
	\centering
...
Die Performance und Verfügbarkeit von \texttt{XDP} hängen vom verwendeten Betriebsmodus ab. 
...
\begin{itemize} 
	\item \textbf{\textit{Native Mode} (\textit{Driver Mode}):} Dies ist der Standardmodus für Hochleistungsanwendungen. 
	Das \texttt{XDP}-Programm wird direkt im Netzwerkkartentreiber ausgeführt. Die Verarbeitung erfolgt nach dem 
	\texttt{DMA}-Transfer (\textit{Direct Memory Access}) in den \textit{Ring-Buffer}, aber vor der \texttt{sk\_buff}-Allokation. 
	Dies erfordert explizite Unterstützung durch den Treiber der Netzwerkkarte. 
	\item \textbf{\textit{Offloaded Mode} (\textit{Hardware Mode}):} Hierbei wird das \texttt{eBPF}-Programm vom Kernel auf die 
	...
	\item \textbf{\textit{Generic Mode} (\textit{SKB Mode}):} Dieser Modus dient der Kompatibilität. Wenn ein Treiber \texttt{XDP} 
	nicht nativ unterstützt, führt der Kernel das \texttt{XDP}-Programm an einer späteren Stelle im Netzwerkstack 
	...
	Trotzdem profitiert es von effizienteren, sperrfreien \textit{Ring-Buffer}-Strukturen (<- TODO bessere Argumente)
\end{itemize}
...
Der Hauptfokus der Sprache ist die Sicherheit. Doch auch Performance und Nebenläufigkeit sind immer weiter in 
...
\textit{Garbage Collectors}, um Speicherfehler, welche das Sicherheitsniveau einer Sprache maßgeblich 
...
\begin{itemize} \label{rust.concepts}
	\item \textbf{\textit{Ownership}:} Jede Variable hat einen \textit{Owner} (Besitzer). Wird der Besitzer gelöscht,
	...
	\item \textbf{\textit{Borrowing}:} Um eine Variable als Referenz in mehreren Kontexten nutzen zu können, 
	ohne dessen \textit{Owner} zu wechseln, gibt es das \textit{Borrowing} Konzept, mit folgenden Regeln:
	es kann entweder eine veränderbare oder mehrere unveränderbare Referenzen einer Variable 
	geben \cite{Bugden_Alahmar_2022}. 
	\item \textbf{\textit{Lifetimes}:} Jede Referenz in Rust besitzt eine Lebensdauer (\textit{Lifetime}), 
    welche den Gültigkeitsbereich definiert, in dem die Referenz valide ist. Meist implizit vom Compiler abgeleitet, 
    verhindern \textit{Lifetimes} falsche Zugriffe, indem sie sicherstellen, dass die referenzierten 
    Daten mindestens so lange existieren wie die Referenz selbst \cite{Bugden_Alahmar_2022}.
\end{itemize}

Die Einhaltung dieser Regeln wird zur Kompilierzeit vom \textit{Borrow-Checker} verifiziert. 
...
\textit{Bounds Checking}, welches auf ungültige Indexzugriffe prüft oder 
...
Außerdem müssen \textit{Pointer-Arithmetik} in sogenannte \texttt{unsafe}-Blöcke ausgelagert werden.
Diese dienen als einer Art Umgehungsmöglichkeit der anderen Konzepte und lösen
einen Programm-beendenden \texttt{panic} aus, welcher verhindert, dass das Programm
...
widerspricht, sind laut Jung et al. "zahlreiche wichtige Rust Bibliotheken" \cite{RustBelt}
sicher, da die \texttt{unsafe}-Blöcke korrekt gekapselt sind \cite{RustBelt}.
...
\subsection{Asynchrone Programmierung und Performance von Rust}
Die im letzten Abschnitt genannten Konzepte schließen auch \textit{data races}, 
welche beim Zugriff mehrerer \textit{Threads} \footnote{Untergeordnete Arbeitseinheiten eines Prozesses} auf 
den gleichen Speicher entstehen können und eine häufige Fehlerquelle in der asynchronen 
Programmierung darstellen \cite{Bugden_Alahmar_2022} bereits zur Kompilierzeit aus \cite{Costanzo_Rucci_Naiouf_Giusti_2021}. 
Dies macht Rust zu einer guten Wahl für sowohl nebenläufige als auch parallele Programmierung. 
...
indem sie während möglicher Wartezeiten z.B. bei I/O-Operationen\footnote{Input-/Output-Operationen}, für andere
Prozesse freigegeben werden. Dadurch kann die Effizienz und die Responsivität des Systems erhöht werden \cite{Silberschatz_Galvin_Gagne_2018, Arpaci-Dusseau_Arpaci-Dusseau_2018}.

Die Parallelität hingegen, bezieht sich auf die tatsächliche physikalische Ausführung mehrerer 
Aufgaben zum gleichen Zeitpunkt \cite{Silberschatz_Galvin_Gagne_2018}, was eine entsprechende \textit{Multi-Core}-Hardware voraussetzt \cite{Concurrency_is_not_parallelism_-_The_Go_Programming_Language}. 
Der Vorteil der Parallelität liegt in der Leistungssteigerung und der Maximierung des 
Datendurchsatzes bei rechenintensiven Problemen \cite{Silberschatz_Galvin_Gagne_2018, Arpaci-Dusseau_Arpaci-Dusseau_2018}.

\subsubsection{Rusts Konzepte zur Performancesteigerung}
Neben den möglichen Performance-Vorteilen durch die nebenläufige Programmierung, welche
aber letztendlich dem Programmierer überlassen ist, bietet die Sprache ihre 
größten internen Performance-Vorteile durch die \textit{Zero-Cost Abstraction}. 
Das Konzept der \textit{Zero-Cost Abstraction} \cite{Cui_Xu_2025}, welches auch in der Sprache C++ Anwendung
findet, kann nach Bjarne Stroustrup wie folgt beschrieben werden: "Was man nicht 
nutzt, dafür bezahlt man nicht. Was man nutzt, könnte man selbst nicht 
besser per Hand codieren" \cite{Stroustrup_1994}.

Dazugehörige Konzepte sind beispielsweise die Eliminierung von Laufzeit-Overhead durch 
die Vermeidung eines zur Laufzeit arbeitenden \textit{Garbage Collectors} \cite{bugden2022safety} \cite{Cui_Xu_2025}, 

...
mithilfe eines eigenem, TCP-Stack im \eng{User-Space}. Dies erlaubt es dem Scanner, die Statusverwaltung und das Timing von Paketen 
...
CPU auf die laufenden Prozesse zu verteilen \cite[S.~737]{Kerrisk_2018}. Die dabei entstehenden Kontextwechsel können das präzise Timing von Hochleistungsanwendungen 
...
und \eng{Zero-Copy} einführte, aber den Netzwerkstack eher ersetzte, statt ihn zu komplementieren, oder \texttt{DPDK} (siehe \ref{Ch3_Alternative_Implementierungsansätze}). 
...
nicht priorisiert wird (siehe \ref{Abgrenzung}). Außerdem ist die Gestaltung der Testumgebung bei beiden Arbeiten nicht auf Hochleistungs-Szenarien ausgelegt. 
...
\begin{itemize}
    ...
    da dieser die historische Basis des Internetscannings darstellt (siehe \ref{Ch3_S1_ZMap}). Da ZMap in C geschrieben ist und auf 
    ...
    eigenen \eng{User-Space}-Stack (siehe \ref{Ch3_Alternative_Implementierungsansätze}) als einer der schnellsten verfügbaren Scanner. 
...
Ein weiteres wesentlicher Nachteil bisheriger Hochleistungsansätze (wie \texttt{DPDK} oder \texttt{PF\_RING}) ist ihre fehlende Integration 
...
dass bei der Einführung und Wartung von \texttt{PF\_RING} für ZMap über die Jahre eine erhebliche Hürde darstellte \cite{Durumeric_Adrian_Stephens_Wustrow_Halderman_2024}. 
...
und durch sein striktes Typ- und Besitzmodell ganze Klassen von Fehlern (wie \eng{Data Races}) eliminiert (siehe \ref{rust}). 
Zusätzlich löst die Nutzung moderner Kernel-Technologien wie \texttt{XDP} und \texttt{eBPF} der Nachteil der fehlenden Kernelintegration 
für die Hochleistungspaketverarbeitung.

Die Kombination dieser Technologien und Werkzeuge stellt in dem Kontext des horizontalen High-Speed-Netzwerkscannings
eine Forschungslücke dar, die in dieser Arbeit untersucht wird.

% TODO im ganzen Projekt: Eigentlich ist
% Technologie falsch, logie = Lehre von etwas
% Also Technologie = Lehre der Technik
% Das ist damit aber gar nicht gemeint. Im Sprachgebrauch meint man meistens Technik

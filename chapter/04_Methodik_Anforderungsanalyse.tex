...
Der Begriff "Performanz-Effizienz" wird
...
Für die Evaluation eines SYN-Scanners lässt sich diese Definition in zwei wesentliche Teilaspekte untergliedern:
\begin{itemize}
    \item \textbf{Zeitverhalten und Durchsatz:} Dies beschreibt die Fähigkeit in einem festen zeitlichen
    Rahmen einen vorgeschriebenen Durchsatz zu erreichen, was für einen SYN-Scanner die Anzahl der gesendeten
    Pakete darstellt.
    \item \textbf{Ressourcennutzung:} Dies beschreibt den Verbrauch von 
    Systemressourcen (z.B. CPU-Zyklen oder Arbeitsspeicher) während der Ausführung. 
\end{itemize}

\subsection{Metriken}
Basierend auf der oben genannten Definition werden folgende Metriken zur Quantifizierung herangezogen:
...
Laboraufbau gewählt. Dieser folgt dem Prinzip einer \textit{Back-to-Back}-Topologie [TODO RFC 2544], bei der zwei 
Systeme über eine direkte Ethernet-Verbindung miteinander kommunizieren.
...
\begin{itemize}
    \item \textbf{Szenario A: Anforderungsvalidierung} Im ersten Szenario wird der Scanner mit einer festen, limitierten Senderate von 100.000 Paketen pro Sekunde (pps) 
...
tatsächlich auf die Hardware- oder Software-Limitierung und nicht auf Implementierungsfehler zurückzuführen sind.
    \item \textbf{Szenario B: Ermittlung der Performanzgrenze} Im zweiten Szenario wird die künstliche Drosselung des Scanners aufgehoben. Der Scanner versucht, Pakete so schnell 

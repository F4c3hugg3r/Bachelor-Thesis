...
% TODO Funktionale und nicht funktionale Anforderungen referenzieren
...
% TODO Formatierung und Anforderungen referenzieren
...
Da der Scanner asynchron arbeiten soll und Sende- und Empfangsprozess entkoppelt sind (siehe /NF-02/), wird 
...
Die Abbildung \ref{fig:component_diag} zeigt die logischen Komponenten des Scanners und deren Interaktionen:
...
in diesem Fall einem \texttt{RingBuf} (siehe \ref{ebpf:xdp_program}) geloggt.
...
Dies bringt große Performanzchancen mit sich bringt.

\subsection{Performancesteigerne Maßnahmen}
Neben der in \ref{ebpf:xdp_program} gezeigten Kernelumgehung werden zur Maximierung des Durchsatzes (/NF-01/) und Minimierung der Ressourcennutzung (/NF-05/) 
...
\begin{enumerate}
    ...
    Zudem wurde beim Design auf die Vermeidung von Kopieroperationen geachtet. Das eBPF-Programm (siehe \ref{eBPF}) arbeitet beispielsweise direkt auf Referenzen 
    ...
    um Kopieroperationen zu vermeiden (siehe \ref{jobverwaltung:parser}).
\end{enumerate}
...
Die Zuordnung der Verzeichnisse zu den logischen Komponenten ist im Diagramm \ref{fig:component_diag} zu finden.
...
Die erfordert im Vergleich zu \texttt{AF\_XDP} oder \texttt{eBPF} mehr Schritte im Netzwerk-Stack (siehe \ref{fig:kernel_diag}).

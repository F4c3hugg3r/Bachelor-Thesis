...
welches die Paketbearbeitung, das \eng{Rate Limiting} und das Versenden übernimmt. Außerdem
\texttt{mock\_programm.rs}, in welchem die Paketrohlinge erstellt und alle Daten zur Konfiguration eingegeben werden.
Für die Aufgabe des Empfangens und Auswerten der Antwortpakete sind zum einen \texttt{xdp-ebpf}, \texttt{xdp-common} und 
\texttt{capturing\_packets} zuständig. Letzteres beinhaltet die Logik zum Empfangen der Daten im \eng{User Space}, welche 
vom \texttt{XDP}-Programm übermittelt wurden und der darauffolgenden Duplikatsentfernung. Die Verzeichnisse \texttt{xdp-ebpf}, \texttt{xdp-common}
beschreiben das \texttt{eBPF}-Programm, welches Antwortpakete abfängt, auswertet und nur die relevanten Informationen an den
\eng{User Space} weiterleitet.
...
Das Verzeichnis \texttt{shared} ist dort nicht aufgeführt, da es lediglich der Steigerung der Übersichtlichkeit dient und
...
von \eng{User Space} und Linux-Kernel nicht explizit behandelt. Um nun aber die Funktion des \texttt{eBPF}-Programmes,
...
XDP-Modi (siehe \ref{Grundlagen.XDP}) beachtet. Im Sendeprozess (von der Anwendung zur Netzwerk-Hardware) wird mithilfe von \texttt{AF\_XDP} der Netzwerk-Stack 
...
Die Ergebnisse der Untersuchung im \texttt{eBPF}-Programm werden bei validen Paketen in eine \texttt{eBPF Map}, in diesem Fall einem \texttt{RingBuf} geloggt.
...
\texttt{tokio} & 1.47.1 & Nutzung für asynchrone Komponenten, Kommunikation über \eng{Channels}, Parsen des \eng{Standard Input} und Starten mehrerer asynchron laufender Tasks \\ \hline
...
\texttt{pnet} & 0.35.0 & Eine abstrahierte Bibliothek für zahlreiche Anwendungen in der Netzwerkprogrammierung \\ \hline
...
Wie in der Abbildung \ref{fig:emitting_sequence_diag} zu sehen, führt der \eng{Rate Limiter} (\texttt{rate\_limiter.rs}) dem Namen entsprechend
die Funktion der Durchsatzlimitierung (Anforderung \hyperref[req:F-08]{/F-08/}) aus. Zuerst ruft er die zu scannenden IP-Adressen vom \eng{Parser} (\texttt{parser\_std\_in})
...
Senderate \eng{Assembler} zu erzeugen, die nicht den gesamten Betriebssystem-\eng{Thread} blockiert, wenn die Pakete eines \eng{Assemblers} nicht zuerst 
...
und der \eng{Batch}-Größe rechnerisch ermittelt (TODO maybe hierauf genauer eingehen). 
...
der Ziel-IP-Adresse, dem Ziel-Port, der \eng{Sequence Number} und berechnet die Checksummen des \eng{IP-} und \eng{TCP-Header} neu. 
Dies dient zur Erfüllung der Anforderung \hyperref[req:F-01]{/F-01/}. Die \eng{Sequence Number} wird wie folgt berechnet:
\begin{equation}
...
In den \eng{Templates} sind die restlichen Werte bereits vorhanden. Die Änderungen werden direkt auf Byte-Ebene umgesetzt, da die Feldzuweisung der 
\eng{Header} immer gleich sind \cite{Eddy_2022,Postel_1981}.
...
\eng{Crate} genutzt (siehe \ref{tab:crates_all}). Der Sender läuft in einer ständigen Schleife bis die \eng{Channels} zum Erhalt
...
zu können und \eng{Context Switches} zu vermeiden. Die Nutzung von \texttt{pcap} stellte die abstrahierte Netzwerkschnittstelle zur Erfüllung der Anforderung \hyperref[req:F-03]{/F-03/} 
dar. Mit \texttt{pcap} muss sich der Programmierer nicht manuell um \eng{Sockets} oder der Kommunikation mit dem Netzwerk-Stack kümmern muss. Ein weiterer Vorteil ist die einfache Nutzung
...
Obwohl die Handhabung mit \texttt{pcap} entwicklerfreundlich ist, wurde es letztendlich durch den \texttt{eBPF}-Ansatz verdrängt, da die \eng{Performance} 
...
\texttt{AF\_INET} nutzt, welches im Vergleich zu \texttt{AF\_PACKET} oder \texttt{AF\_XDP} deutlich mehr Schritte im Netzwerk-Stack durchlaufen muss (siehe \ref{fig:kernel_diag}),
...
muss dieser in das asynchrone Modell der Anwendung integriert werden.  Dafür bietet der \texttt{tokio} \eng{Crate} eine Lösung, welche es ermöglicht, 
...
\eng{Standard Output} geschrieben. Somit wird Anforderung \hyperref[req:F-07]{/F-07/} erfüllt.
...
wurde und automatisch eine \texttt{RST}-Antwort zurückschickt [TODO Quelle?].
...
Um die Anforderung \hyperref[req:F-09]{/F-09/} zu erfüllen, nimmt der Scanner die IP-Adressen der Ziele über den \eng{Standard Input} entgegen.
...
Die \eng{Ethernet-Templates}, welche das Fundament zur Erfüllung der Anforderung \hyperref[req:F-01]{/F-01/} darstellen,
...
Es wird für jede Quell-IP ein \eng{Template} angelegt, da diese der Streuung der Paketquellen zur Verschleierung des Scans
und somit der Erhöhung der Trefferrate dienen.

Um möglichst ressourcensparend zu arbeiten werden die Ziel-IP-Adressen in \eng{Batches}, also Ansammlungen von Paketen übertragen.
Dies reduziert die Anzahl der benötigen \eng{Context Switches} drastisch indem nicht für jede IP-Adresse ein eigener \eng{System Call} \footnote{TODO} getätigt werden muss.
...
\subsubsection{\eng{Standard-Input} \eng{Parser} (\texttt{parser\_std\_in})}
Der \eng{Parser} parst zum einen die Konfigurationsparameter aus dem \eng{Standard Input} und zum anderen die Ziel-IP-Adressen. Das Entgegennehmen, Deserialisieren
und Weiterleiten der Adressen geschieht in \eng{Batches} und mit Fokus auf der Vermeidung neuer Allokationen um durch Einsparung von Kopien und \eng{Context Switches}
durch \eng{System Calls} Ressourcen zu sparen. Dabei wird darauf geachtet die Daten, falls möglich, mit \eng{Zero-Copy}-Operationen zu verarbeiten. 

Der folgende Codeauszug \ref{lst:parser_binary} zeigt das \eng{Parsing} der Daten welche im Binärformat übertragen wurden. 
...
\section{\texttt{eBPF}}
Der \texttt{eBPF} dient dem Scanner als Empfangspunkt für eingehende Pakete. Je nachdem in welchem \texttt{XDP}-Modus das Programm ausgeführt wird, agiert dieses direkt 
...
in das \eng{User Space}-Programm übertragen werden (siehe \ref{fig:kernel_diag}). 
...
dadurch enorm. Die zum Informationsaustausch zwischen \texttt{eBPF}-Programm und \eng{User Space}-Programm genutzten \texttt{eBPF Maps} werden in \ref{tab:ebpf_maps} aufgeführt.

\begin{table}[htbp]
\caption{Genutzte \texttt{eBPF Maps}}
\begin{tabularx}{\textwidth}{|c|c|X|}\hline 
\textbf{Name} & \textbf{Typ} & \textbf{Nutzung} \\ \hline
\texttt{STATS} & \texttt{PerCpuArray<u64>} & Lockfreies \eng{Array} für jede CPU auf welchen \eng{Stats} effizient geloggt werden können \cite{Map_Type_“BPF_MAP_TYPE_PERCPU_ARRAY”_-_eBPF_Docs}\\ \hline
\texttt{WHITELIST\_IPV4} & \texttt{HashMap<[u8; 4], u8>} & \eng{HashMap} zum Abgleich der Quell-IP-Adressen, welche für den Scan genutzt werden \\ \hline
\texttt{EVENTS} & \texttt{RingBuf} & Ausgabe der extrahierten Zielinformationen valider Pakete an den \eng{Receiver} mithilfe eines effizienten, über alle CPUs geteilten Puffer-Rings \cite{Map_Type_“BPF_MAP_TYPE_RINGBUF”_-_eBPF_Docs}\\ \hline
\texttt{SIPHASH\_KEY} & \texttt{Array<u64>} & Der Schlüssel wird benötigt, um den \eng{SYN-Cookie} korrekt auszuwerten \\ \hline
\end{tabularx}
\label{tab:ebpf_maps}
\end{table}
...
Wie in Abbildung \ref{fig:ebpf_component_diag} zu sehen, wird aus allen empfangenen Paketen zuerst der \eng{Ethernet-}, \eng{IP-} und \eng{TCP-Header} extrahiert und 
...
mithilfe des \eng{Sip-Hashing}-Algorithmus und des per \texttt{SIPHASH\_KEY} übergegeben Schlüssels durchgeführt. Die vier in \ref{equqasion} genannten 
...
des \eng{Offsets} aus bis zum Ende des Speicherbereichs zurückgegeben werden.
...
Dabei ist zu beachten, dass diese Aufrufe von einem \texttt{unsafe}-Block umschlossen sind. Da dieser Speicher vom Linux-Kernel und nicht von der Rust-Runtime 
...
angestrebt wird. Dies hängt damit zusammen, dass Rust normalerweise mit einem \texttt{panic!} regiert, sollte beispeilsweise auf einen falschen Index eines
...
\eng{Performance} deutlich und ist somit für performanz-orientiere Anwendungen wie die hier implementierte, besonders im Kernel-Kontext, in welchem nur 

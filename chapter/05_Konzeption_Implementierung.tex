...
% TODO Auch nicht funktionale Anforderungen mit einbringen
...
\subsubsection{Effizienzsteigernde Maßnahmen}
% TODO für Sender, RateLimiter und Assembler in einem Absatz sagen, dass immer batches über tokio channel übertragen wurden 
% TODO sagen, dass im Allgemeinen zum Sparen von mallocs vecs geleert wurden und vecs immer mit fester Größe
...
% (maybe den Absatz vom Ratelimiter mit tokio auch hier hin umziehen)
...
die Funktion der Durchsatzlimitierung (Anforderung \hyperref[req:F-08]{/F-08/}) aus. Zuerst nimmt er die zu scannenden IP-Adressen vom \eng{Parser} (\texttt{parser\_std\_in})
...
% TODO maybe mathematische Formel ergänzen. 
% oder auf checkcurrent_sent_bytes genauer eingehen
...
% TODO Batch Sending erwähnen und oder wahlweise flowchart für Senden mit xdp / afp einfügen 
...
% TODO am besten aus Projektaufbau entfernen 
...
\subsubsection{\eng{Standard Input Parser} (\texttt{parser\_std\_in})}
...
Im bereitgestellten Codeauszug wurden explizit die in \ref{tab:parser-optimizations} beschriebenen Performanz-steigernden Maßnahmen umgesetzt.
...
mithilfe des \eng{Sip-Hashing}-Algorithmus und des per \texttt{SIPHASH\_KEY} übergegeben Schlüssels durchgeführt. Die vier in \ref{equ:syn_cookie_umsetzung} genannten 
Werte werden wie in \ref{equ:syn_cookie_auswertung} beschrieben verrechnet und ausgewertet, um die Anforderung \hyperref[req:F-05]{/F-05/} 
zu erfüllen.
...
entsprechender Werte und das Neuberechnen einiger Werte umgewandelt wird. Die Veränderungen der Felder sind in Abbildung \ref{fig:rst_packet_creation} beschrieben.
...
% Notwendig, wenn ich eine ausführliche Evaluation mit Funktionstests mache?
\section{Qualitätssicherung}
% Als Maßnahmen zur Qualitätssicherung wurden im Entwicklungsprozess Integrationstests mithilfe der in \ref{}
% genannten Tools durchgeführt. 

...
\section{Programmstart und Jobverwaltung (job\_controlling)}
...
\subsection{Inkompatibilitäten und Einschränkungen}
% TODO hier text schreiben

\subsubsection{zero-copy Modus}
...
Aufgrund dessen wird in den entgültigen Test der Copy Mode verwendet, welcher dieses Problem, durch die Entkopplung der Speicherverwaltung von der Netzwerkkarte eleminiert.
Dies kommt leider mit erheblichen Performanzeinbußen, weshalb alle Tests ein zweites Mal ohne dem Senden der RST-Antworten durchgeführt werden, um die mit kompatibler 
Hardware möglichen Ergebnisse zu beachten.

\subsubsection{Paketrate}
...
bei einer Paketgröße von 64 Byte plus 20 Byte Overhead, bei 1,488 Millionen pps. 

\subsection{Aufbau des Zielknotens}
...
%Immer gleich: 5 Durchläufe pro Scanner, 20% Antwortrate, Src IP Raum /26, rx und tx 4096 

%Durchlauf 1: POC 
%/10 IP Raum, 500.000pps, 20% Antwortrate, RST senden, port 80,443 dedup, 100 src_ports

%SYN-Rust (XDP, Copy): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_c_1

%SYN-Rust (XDP, Zero-Copy, kein RST): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_zc_1

%SYN-Rust (AF_PACKET): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_afp_1

%ZMap: zmap -q -p 80,443 -r 500000 -i enp6s0 -G 00:1b:21:da:fc:ef -S 192.168.0.0-192.168.0.63 -O json -o output_results.csv --source-port 60000-60100 --dedup-method=full 1.0.0.0/10

%Masscan (kein dedup): masscan 1.0.0.0/10 -p80,443 --rate 500000 -e enp6s0 --router-mac 00:1b:21:da:fc:ef --adapter-ip 192.168.0.0-192.168.0.63 -oJ results.json --source-port 60000-60100


%Durchlauf 2: Performance-Effizienz
%/5 IP Raum, ohne RST, ohne dedup, port 80, src_port 60000

%SYN-Rust (XDP, Copy): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_c_2

%SYN-Rust (XDP, Zero-Copy): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_zc_2

%SYN-Rust (AF_PACKET, 2 Threads): /home/lennard/Bachelor/SYN-Rust/target/release/mock_program_afp_2

%ZMap: zmap -q -p 80 -B 2000M -T 1 -i enp6s0 -G 00:1b:21:da:fc:ef -S 192.168.0.0-192.168.0.63 -O json -o output_results.csv --source-port 60000 1.0.0.0/5

%Masscan: masscan 1.0.0.0/5 -p80 --rate 2000000 -e enp6s0 --router-mac 00:1b:21:da:fc:ef --adapter-ip 192.168.0.0-192.168.0.63 -oJ results.json --source-port 60000

%# Droppe eingehende Pakete auf den Scan-Ports, damit der Kernel sie nicht sieht.
%sudo iptables -A INPUT -p tcp --dport 60000 -j DROP

% TODO je nachdem wie viel Platz noch ist hier mglw. ein kleinen Flowchart mit Ablauf hin

\subsection{Messdatenerfassung}

% Benchmarkprogramm: Ablauf und Messtechnik; Validierungstechnik (Validierungstests)

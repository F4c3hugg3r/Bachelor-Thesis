...
die Softwarearchitektur nach dem \eng{Producer-Consumer-Pattern} entworfen. Der Datenfluss wird dabei als \eng{Pipeline} betrachtet: 
Die Kommunikation zwischen den Modulen erfolgt über sogenannte \eng{Channels}. Die jeweilige Komponente erhält Daten über einen \eng{Channel}, 
verarbeitet diese und schreibt die Ergebnisse in einen anderen \eng{Channel}, welcher gleichzeitig als Puffer dient, um Lastspitzen auszugleichen. 
Diese Modularisierung ermöglicht es, dass \eng{I/O}-lastige Operationen (wie das Lesen vom \eng{Standard Input}) die CPU-lastigen 
...
\subsection{Performancesteigerne Maßnahmen} \label{impl:performance_steigernde_maßnahmen}
...
Für die Umsetzung der Komponenten sind die in \cref{tab:crates_all} beschriebenen \eng{Crates} aufgrund ihres Einflussreichtums hervorzuheben.
...
% wahlweise TODO wahlweise kleinen Flowchart
...
\texttt{tokio} \eng{Tasks} oder auch \eng{Green-Threads} sind kleine Ausführungseinheiten, ähnlich eines \eng{Threads} des Betriebssystems, bloß dass diese 
...
benötigen und erlauben asynchrone Ausführung mehrerer \eng{Tasks}, da sie, statt wie \eng{Threads} des Betriebssystems zu blockieren, die Ressourcen
...
Senderate \eng{Assembler} zu erzeugen, die nicht den gesamten \eng{Thread} des Betriebssystems blockieren, wenn die Pakete eines \eng{Assemblers} nicht zuerst 
...
% wahlweise TODO mathematische Formel ergänzen oder auf checkcurrent_sent_bytes genauer eingehen
...
einer \eng{Hashing}-Funktion entsprechend bei gleichem Input immer den gleichen Wert zurückgibt \cite{SipHash_a_short_input_PRF_The_Linux_Kernel_documentation}.
...
% wahlweise TODO hier flowchart einfügen 

\subsubsection{\eng{Sender} (\texttt{sender.rs})}
Der \eng{Sender} agiert im Kontrast zu den anderen Subkomponenten in einem eigenen 
\eng{Thread} des Betriebssystems. Das hat den Grund, dass er somit die komplette Kapazität des \eng{Threads} 
...
% wahlweise TODO flowchart für Senden mit xdp / afp einfügen 
...
So wird ein asynchroner \texttt{tokio}-\eng{Reader} eingesetzt, um zu gewährleisten, dass der Einleseprozess auch bei ausgelasteten 
...
Prinzipien werden zudem \eng{Batching} und \eng{Zero-Copy}-Techniken angewandt. Letzteres wird unter anderem durch die Nutzung von 
\texttt{std::mem::replace} realisiert, womit Speicherpuffer effizient durch den Austausch von \eng{Ownership} verwaltet werden, 
statt kostenintensive Kopieroperationen durchzuführen. Um auch bei fragmentierten Datenübertragungen keine Informationen zu verlieren, 
...
%
%% maybe TODO (Buffersize erklären (L1 Cache?)) 
...
Um ein \texttt{eBPF}-Programm nutzen zu können wird es in die \texttt{XDP}-\eng{Hook} des Kernels geladen und zuvor korrekt in \eng{ELF}-Dateien übersetzt, damit die \texttt{XDP}-\eng{Hook}
...
Wie in \cref{fig:ebpf_component_diag} zu sehen, wird aus allen empfangenen Paketen zuerst der \eng{Ethernet-Header}, \eng{IP-Header} und \eng{TCP-Header} extrahiert und 
überprüft. Sollte dabei festgestellt werden, dass ein Paket kein valides \eng{IPv4}-\texttt{SYN-ACK}-Paket darstellt, wird es per \texttt{XDP\_PASS} an den normalen 
...
in der \texttt{WHITELIST\_IPV4} vertreten ist oder die Prüfung des \texttt{SYN}-Cookies führt bei Fehlschlag zu einem \texttt{XDP\_PASS}. Das bietet den Vorteil, 
dass der reguläre Netzwerkverkehr trotz Nutzung des \texttt{SYN}-Scanners unbeeinträchtigt ist. Um den \texttt{SYN}-Cookie zu vergleichen wird die Hash-Berechnung 
mithilfe des \eng{SipHash}-Algorithmus und des per \texttt{SIPHASH\_KEY} übergegeben Schlüssels durchgeführt. Die vier in \cref{equ:syn_cookie_umsetzung} genannten 
...
Die Entwicklung des \texttt{eBPF}-Programms unterliegt architektonischen Restriktionen (vgl. \cref{Grundlagen.eBPF}). 
Da der Code im \eng{Kernel-Space} ausgeführt wird, stehen keine Betriebssystemdienste zur Verfügung, die 
in der Regel per \eng{System Call} aufgerufen werden \cite[S.~59]{Rice_2023}. Eine wichtige Konsequenz ist die Abwesenheit eines globalen \eng{Memory Allocators}, wodurch 
dynamisch wachsende Datenstrukturen wie \texttt{Vec<T>}, \texttt{String} oder \eng{HashMaps} nicht verwendbar sind \cite[S.~206]{Rice_2023}. 
Um unter diesen Bedingungen trotzdem eine typsichere Verarbeitung zu gewährleisten, wird das \texttt{network\_types}-\eng{Crate} eingesetzt. 
Diese Bibliothek ermöglicht es, rohe Speicherbereiche direkt durch \eng{Structs} und \eng{Enums} zu abstrahieren, anstatt sie in neue Datenstrukturen zu parsen. 
Dies erlaubt das Lesen und Modifizieren von Paket-\eng{Headern} ohne die dahinterliegenden Daten zu kopieren.
...
% maybe TODO anmerken, dass der Code von aya ist, caption bei code auf Unterseite legen (AI fragens) 
...
die bei Fehlern in einem Programmabbruch (\texttt{panic!}) resultieren. Da ein solcher Abbruch im Kernel-Kontext unzulässig ist 
und das alternative Anlegen einer sicheren Speicherkopie nicht den Performanzzielen dieser Arbeit entsprechen würde, 
wird von den Vorteilen aus den spezifischen Restriktionen der \texttt{eBPF}-Laufzeitumgebung (vgl. \cref{Grundlagen.eBPF}) Nutzen ergriffen. 
Diese gelten trotz roher \texttt{unsafe}-Zeigeroperationen, weshalb die Speichersicherheit dennoch gewahrt ist.
...
Strukturierung, während der \eng{Verifier} die Rolle der Instanz übernimmt, die die Speichersicherheit durchsetzt.
...
Das anschließende Erstellen und Versenden des \texttt{RST}-Paketes dient dazu, die Verbindung beim Zielsystem korrekt zu schließen und somit einen 
normalen TCP-Aushandlungsprozess zu simulieren. Außerdem hindert es das Zielsystem am Senden weiterer \texttt{SYN-ACK}-Antworten sowie dem Aufrechterhalten
eines Verbindungsstatus. Um die \eng{Performance} zu erhöhen wird auch hier ein \eng{Zero-Copy}-Ansatz gewählt, indem das ursprüngliche Paket durch das Tauschen

...
% TODO DARAUF EINGEHEN WELCHE ANFORDERUNGEN DURCH WAS GETESTET WERDEN
...
wurden für die Szenarien (siehe \ref{Methodik}) Python-Programme erstellt, mit
...
Für das Szenario 1 (siehe \ref{Methodik}) werden die Scanausgaben, sowie die in \ref{tab:tools_validation} beschriebenen Tools genutzt, 
um zusätzlich zu den Ausgaben des \eng{Dummy-Receiver}-Programmes exakte Validierung anstellen zu können. 
...
tcpdump & TODO \\ \hline
xdpdump & TODO \\ \hline
Wireshark & TODO \\ \hline
\end{tabularx}
\caption{Genutzte Tools zur Validierung der funktionalen Anforderungen (siehe \ref{Methodik})}
\label{tab:tools_validation}
\end{table}

In den Szenarien 2 und 3 erfolgt die Messung der Metriken (siehe \ref{Methodik}) mit einer Abtastrate von $10\,\text{Hz}$ (Intervall $t=0,1\,\text{s}$). 
...
Die \texttt{benchmark\_suite.py} \ref{TODO anhang} startet die Scanner-Prozesse nacheinander und misst mit einem parallelen \eng{Monitoring}-\eng{Thread} die
...
In Szenario 1 \cite{TODO} wird zur Aswertung statt der Metriken der \texttt{benchmark\_suite.py} ein eigenes Programm (\texttt{validate\_scanner.py \cite{TODO}})
genutzt. Für die Messungen zur Validierung der Pakete wird gar kein automatisierter Ablauf genutzt, da aufgrund der Nutzung von Wireshark \ref{tab:tools_validation}
eine manuelle Auswertung erfolgt.
...
Der erste Test dient als \eng{Proof of Concept} des Scanners. Hierbei wird ein kleiner IP-Adressraum (\texttt{/20}) gescannt.
...
Dabei wird das Senden von \texttt{RST}-Paketen aktiviert. 

Im zweiten manuellen Test, werden nur jeweils 4 Pakete verschickt. Dies genügt, um zu erkennen, ob die Pakete korrekt sind und fördert
die Übersichtlichkeit. Die Ergebnisse werden mithilfe der in \ref{tab:tools_validation} beschriebenen 
Netzwerktools ausgewertet.

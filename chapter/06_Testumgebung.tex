...
Um die Scanergebnisse zu plausibilisieren wird zusätzlich via \texttt{validate\_responses.py} \cref{TODO anhang} die Anzahl der ausgegebenen Ergebnisse gezählt. 
Die Rohdaten werden nach der Messung mithilfe des \texttt{plot\_benchmark\_suite.py} \cref{TODO anhang} statistisch bereinigt und visualisiert. 
...
\begin{itemize}
    ...
    \item \textbf{\eng{IP}-Raum:} Als Ziel dient ein \texttt{/6}-Netzwerk\footnote{67.108.864 Millionen \eng{IP}-Adressen}, 
...
\begin{itemize}
    ...
    \item \textbf{\eng{IP}-Raum:} Der Zielbereich wird auf ein \texttt{/10}-Netzwerk\footnote{4.194.304 Millionen \eng{IP}-Adressen} beschränkt, 
...
Diese Sektion beleuchtet deren Auswirkungen auf die Testdurchführung, insbesondere im Hinblick auf den \eng{Zero-Copy}-Modus 
...
Die Durchsatzrate ist durch die genutzten Netzwerkkarten sowie dem \eng{LAN}-Kabel auf das Limit einer Gigabit-Verbindung beschränkt. Die Tests zur maximalen Durchsatzrate 

...
Mittels dessen und der \texttt{README.md}-Datei können die \eng{Benchmarks} exakt reproduziert und nachvollzogen werden. 

\subsection{Ergebnisse Evaluationstests: \eng{Proof of Concept}} \label{Ergebnisse.PoC}
...
Beim \cref{req:T-02} zeigten ebenfalls alle Variationen das gleiche Verhalten. Die \cref{tab:results_s02_quant} steht also stellvertretend für alle Testabläufe.
Die Ergebnisse für den Ziel-Knoten sind in den Dateien \texttt{capture\_no\_rst.pcap} und \texttt{capture\_with\_rst.pcap} \cite{TODO} zu finden. Die des
Scanner-Knotens in \texttt{xdpdump\_no\_rst.pcap} und \texttt{xdpdump\_with\_rst.pcap} \cite{TODO}.
...
keine Option zur Vermeidung des Sendens von \texttt{RST}-Antworten besitzen. Da Masscan die Pakete in dessen eigens gefertigtem \eng{User-Space}-TCP-Stack
erstellt und versendet, fließen diese auch in die \eng{PPS}-Metrik ein. Die durch ZMap bedingten \texttt{RST}-Antworten werden automatisch vom Kernel 
...
sind in [TODO Ergebnisse] und [TODO Validierung] zu finden. Für ZMap gilt bezüglich der \texttt{RST}-Antworten das gleiche wie auch in \cref{Ergebnisse.S-01}.  
...
Analyse der Performanzeffizienz im Vergleich zu bestehenden Lösungen.
...
Modi, in welchen diese Ausgabe zu beobachten ist, am Ende des Scans ein paar invalide Pakete über den \eng{Socket} verschickt. Bei vorherigen Tests wurden 
...
Die Ergebnisse zu \cref{req:T-02} zeigen, dass die \texttt{SYN}-Pakete korrekt gebaut werden, da Netcat und Wireshark sie als valide anerkennen. Auch die 
...
Durchsatz und Ressourceneffizienz unter Volllast sowie in realitätsnahen Szenarien, um die Vorteile der genutzten Technologien 
zu quantifizieren.
...
Dies unterstreicht die Effizienz der genutzten \eng{Kernel-Bypass}-Technologien \texttt{AF\_XDP} und \texttt{eBPF}. Besonders im \eng{Zero-Copy}-Modus sind deutliche Effizienzgewinne in Form einer
...
des Rust Scanners (SYN-Rust), welche den \eng{Copy Mode} des \texttt{AF\_XDP}-\eng{Sockets} nutzen, zeigen eine deutliche Effizienzsteigerung zu den externen Vergleichsobjekten. Die \texttt{AF\_PACKET}-Konfiguration
von SYN-Rust ist bezüglich der Effizienz und Gesamtlast der CPU mit Masscan vergleichbar. Das ist ein sehr interessantes Ergebnis, da Masscan zum einen 
einen eigenen TCP-Stack implementiert, um den Kernel zu umgehen und zum anderen die Nutzung von \texttt{AF\_PACKET} durch die Nutzung eines gemeinsamen Speicherbereiches von 
\eng{User Space} und \eng{Kernel Space} drastisch optimiert hat. Die SYN-Rust Variante implementiert keine dieser Optimierungen.

Der einzige Punkt, in welchem die Vergleichsobjekte vorne liegen, ist der RAM Verbrauch (siehe \ref{fig:ram_efficiency_diag}). ZMap und Masscan zeigen nahezu 
keinen RAM-Verbrauch während die Rust-Implementierungen einen moderaten Verbrauch haben, der im Zuge heutiger Kapazitäten gering ausfällt. %TODO warum? (siehe Copilot chat pc)

Anhand von Abbildung \ref{fig:cpu_efficiency_diag}, welche die Verteilung der CPU Gesamtlast auf die verschiedenen Bereiche \eng{User Space}, \eng{Kernel Space} und \eng{SoftIRQ} verbildlicht, lässt 
...
niedrigeren. Dies zeigt die Effizienz der \eng{Kernel-Bypass}-Technologie von \texttt{AF\_XDP}. Die Scanner, die \texttt{XDP} im \eng{Native Mode} nutzen, zeigen eine verminderte \eng{SoftIRQ}-Auslastung, da Schritte wie das Erstellen
eines \texttt{sk\_bufs} vermieden werden. An der \eng{User-Space}-Auslastung ist zu erkennen, dass die Maßnahmen zur Effizienzsteigerung (vgl. \ref{Implementierung}) wirkungsvoll sind. Auch die fehlende
Notwendigkeit, empfangene Pakete im \eng{User Space} parsen zu müssen, da es schon im \texttt{eBPF}-Programm getan wird, spielt dem zu.
...
Auslastung des Programmes bei. Die Ausgabe der Ergebnisse von Masscan erfolgte in eine \texttt{.txt}-Datei statt in eine \texttt{CSV}-Datei wie bei den anderen Scannern.
...
\texttt{RST}-Antworten zuzuschreiben. Andererseits könnte es auch bedeuten, dass diese bei einer niedrigeren Durchsatzlimitierung die Ressourcen ein wenig schlechter verwalten. Trotzdem
sind die Ergebnisse sehr gut und die RAM-Auslastung skaliert den Ergebnissen nach gut bei gedrosselten Raten. Interessant ist, dass die \texttt{AF\_PACKET}-Konfiguration bezüglich der
...
In \cref{tab:performance_comparison_low_rate_efficiency} ist zu sehen, dass die Rust Scanner mit 1,64 Mio Ergebnissen rund 2,4\,\% weniger Ergebnisse als die erwartete Menge erkannt haben.

$2^28 (IP-Adressraum) * 0,2 (Antwortrate) * 2 (Ports) = ~1.68 Mio$ % TODO in idiomatische Formel umwandeln

In der \texttt{ethtool.csv}-Datei \cite{TODO} ist zu sehen, dass die Gesamtanzahl der versendeten Pakete (inklusive \texttt{RST}-Pakete) sowohl beim Zähler der Netzwerkkarte, als auch beim Zähler 
von \texttt{procfs}, um rund 2,3\,\% vom erwarteten Wert (10.066.330) abweicht. Aufgrund dieser Informationen und da dieses Problem in keinem anderen der Tests auftrat, ist es wahrscheinlich, 
...
\begin{table}[htbp]
    ...
    Technologische Einschränkung & Erfüllt & Keine proprietären Treiber verwendet (vgl. \cref{Implementierung}) \\ \hline
    \end{tabularx}
    \begin{tablenotes}
...
\begin{itemize}
    ...
    Für den produktiven Einsatz ist jedoch auch die Scangenauigkeit von kritischer 
    Bedeutung, wie bereits in verwandten Arbeiten dargelegt wurde \cite{TODO}. Eine weiterführende Untersuchung sollte analysieren, 
    ...
    \item \textbf{Hardwareanpassung und -Skalierung:} Wie in \cref{Inkompatibilitäten} diskutiert, stellte der verwendete 
    Netzwerkkartentreiber sowie die Hardwarelimitierung auf 1 Gbit/s (\cref{tab:specs_combined}) einen Flaschenhals dar 
    ...
    die zunehmende Verbreitung an Relevanz gewinnt \cite{TODO}, stellt die Anpassung des Scanners, sodass auch dieses Protokoll 
    unterstützt wird, einen logischen nächsten Schritt dar. Hierbei wäre insbesondere zu untersuchen, wie sich die vergrößerten 
    Adressstrukturen (128 Bit) auf die Speicherverwaltung im \texttt{eBPF}-Programm, der veränderte \eng{Parsing}-Aufwand aufgrund von \eng{Next Header}-Verkettungen und die damit verbundene Rechenlast für \eng{Hashing}-Operationen auf den Durchsatz, sowie die Effizienz im Kontext
    eines \texttt{SYN}-Scanners auswirken.

    \item \textbf{Adaption auf weitere Scan-Methoden:} Die in dieser Arbeit entwickelte Architektur zur Kernelumgehung ist 

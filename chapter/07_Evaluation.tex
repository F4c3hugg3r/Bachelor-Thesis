...
Ausgaben der Tests, lässt sich dort unter \texttt{logs\_benchmark\_suite} und \texttt{logs\_dummy\_receiver} nachvollziehen. 
Mithilfe dieser Dateien und der \texttt{README}-Datei können die \eng{Benchmarks} exakt reproduziert und nachvollzogen werden. 

% maybe TODO als Text darstellen und Tabellen in Anhang packen
\subsection{Ergebnisse der Evaluationstests: Proof of Concept} \label{Ergebnisse.PoC}
...
Aus den Ergebnissen des Tests zum \cref{req:T-02}\cite{F4c3hugg3r_2026} erschließen sich nach Evaluationsszenario\_1/zusammenfassung\_ergebnisse\_final und Evaluationsszenario\_1/validierung\_hits die
...
sind in Evaluationsszenario\_2/zusammenfassung\_ergebnisse\_final und Evaluationsszenario\_2/validierung\_hits\cite{F4c3hugg3r_2026} zu finden. 
...
Dabei ist anzumerken, dass in \texttt{log\_dummy\_receiver} zu sehen ist, dass mehr als die
...
\textbf{Steigerung der Performance-Effizienz:} Gemäß der Ergebnisse des \cref{req:S-01} ist eine deutliche Steigerung der \eng{Performance}-Effizienz gegenüber den bewährten Vergleichsobjekten ZMap und Masscan zu erkennen.
...
im Gegensatz zu den Vergleichsobjekten keine \eng{Busy Loop}\footnote{Es wird dauerhaft ohne Verzögerung Kapazität abgefragt.} beim Senden, sondern nutzen wie in \cref{fig:send_af_xdp} zu sehen einen linearen 
...
\textbf{Analyse des Durchsatzes:} Bezüglich des absoluten Durchsatzes erreichte lediglich der \eng{Zero-Copy}-Modus das physikalische Limit der Gigabit-Verbindung. 
...
\textbf{Ressourcenverbrauch (RAM):} Die Vergleichsobjekte zeigen bezüglich des RAM-Verbrauchs bessere Ergebnisse als die Rust-Implementierung (siehe \cref{fig:ram_efficiency_diag}). ZMap und Masscan zeigen nahezu 
...
\textbf{CPU-Lastverteilung:} Anhand von \cref{fig:cpu_efficiency_diag}, welche die Verteilung der CPU-Gesamtlast auf die verschiedenen Bereiche \eng{User-Space}, \eng{Kernel-Space} und \eng{SoftIRQ} verbildlicht, lässt 
...
\textbf{Methodische Unterschiede:} Um die Ergebnisse in das korrekte Verhältnis zu setzen, ist es wichtig, die Unterschiede zwischen der Funktionsweise aufzuzeigen. Im Gegensatz zu ZMap und Masscan hat SYN-Rust
...
\textbf{Effizienz bei Drosselung:} In \cref{req:S-02} schlossen die SYN-Rust Varianten, die \texttt{XDP} nutzen, schlechter ab als zuvor. Dies ist einerseits den aktivierten \eng{Features} wie der Duplikationserkennung und dem Senden von 
...
\textbf{Eingeschränkte Vergleichbarkeit:} Die Ergebnisse der Effizienz von ZMap und Masscan sind in diesem Fall nur bedingt mit den restlichen (bis auf \eng{Zero-Copy}) vergleichbar, da SYN-Rust eine andere Designphilosophie  
...
\textbf{Analyse der Abweichungen:} In \cref{tab:performance_comparison_low_rate_efficiency} ist zu sehen, dass die Rust Scanner mit rund 1,64 Millionen Ergebnissen rund 2,4\,\% weniger Ergebnisse als die erwartete Menge 
...
Laut der Datei Evaluationsszenario\_2/ethtool\_ergebnisse\cite{F4c3hugg3r_2026} weicht die Gesamtanzahl der versendeten Pakete (inklusive \texttt{RST}-Pakete) sowohl beim Zähler der Netzwerkkarte, als auch beim Zähler 
...
Einschränkend muss angemerkt werden, dass die Hardwarelimitierung der Testumgebung eine Ermittlung der maximalen Durchsatzgrenze, 
sowie das Senden von RST-Paketen im 
